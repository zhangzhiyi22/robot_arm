...
class MoveItCartesianDemo:
    def __init__(self):
        ...
        self.arm.set_max_velocity_scaling_factor(0.1)  # 将速度缩放因子设置为最大值1.0
        self.arm.set_max_acceleration_scaling_factor(0.1)  # 将加速度缩放因子设置为最大值1.0
        ...
        self.vertices = [
            ...
            [self.start_pose.position.x, self.start_pose.position.y + 0.1, self.start_pose.position.z],  # 顶点 3 (沿Y轴正方向)
            [self.start_pose.position.x, self.start_pose.position.y, self.start_pose.position.z + 0.1]  # 顶点 4 (沿Z轴正方向)
        ]

        # 定义正四面体的边（顺序包括重复点以形成闭合路径）
        self.edges = [
            (0, 1), (1, 2), (2, 0),  # 底面三条边
            (0, 3), (3, 1), (1, 3), (3, 2), (2, 0)  # 从底面到顶点的边
        ]
